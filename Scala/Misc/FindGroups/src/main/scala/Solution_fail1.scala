...
object Result_fail1 {
  ...
  def countGroups(related: Array[String]): Int = {
    ...
    //    loop
    //    i from 1 to n-1
    //    if (i-1)(i) or (i)(i-1) or (i+1)(i) or (i)(i+1) is 1, then make both 4 of them all 1
    // 3. init g = 1
    // 4. loop
    //    i from 1 to n-1
    //    j from 0 -> i-1
    ...
    //      if (relatedCopy(i - 1)(i) == '0' && relatedCopy(i)(i - 1) == '1') {
    //        // update relatedCopy(i-1)(i) to '1'
    //        //relatedCopy(i-1)(i) = '1'
    //        println(s"Need to update relatedCopy(${i - 1})($i) from ${relatedCopy(i - 1)(i)} to 1")
    //        println(s"relatedCopy(${i - 1}) before: ${relatedCopy(i - 1)}")
    //        relatedCopy(i - 1) = relatedCopy(i - 1).slice(0, i) + '1' + relatedCopy(i - 1).slice(i, pplCount)
    //        println(s"relatedCopy(${i - 1}) after: ${relatedCopy(i - 1)}")
    //      } else if (relatedCopy(i - 1)(i) == '1' && relatedCopy(i)(i - 1) == '0') {
    //        // update relatedCopy(i)(i-1) to '1'
    //        println(s"Need to update relatedCopy($i)(${i - 1}) from ${relatedCopy(i)(i - 1)} to 1")
    //        println(s"relatedCopy($i) before: ${relatedCopy(i)}")
    //        relatedCopy(i) = relatedCopy(i).slice(0, i - 1) + '1' + relatedCopy(i).slice(i, pplCount)
    //        println(s"relatedCopy($i) after: ${relatedCopy(i)}")
    //      }
    //    }
    ...
    for (i <- 1 until pplCount) {
      ...
      if ( (i > 0 && relatedCopy(i - 1)(i) == '1')
        ...
        if (i > 0 && relatedCopy(i - 1)(i) == '0') {
          // update relatedCopy(i-1)(i) to '1'
          println(s"Need to update relatedCopy(${i - 1})($i) from ${relatedCopy(i - 1)(i)} to 1")
          println(s"relatedCopy(${i - 1}) before: ${relatedCopy(i - 1)}")
          relatedCopy(i - 1) = relatedCopy(i - 1).slice(0, i) + '1' + relatedCopy(i - 1).slice(i + 1, pplCount)
          println(s"relatedCopy(${i - 1}) after: ${relatedCopy(i - 1)}")
        }
        if (i > 0 && relatedCopy(i)(i - 1) == '0') {
          // update relatedCopy(i)(i-1) to '1'
          println(s"Need to update relatedCopy($i)(${i - 1}) from ${relatedCopy(i)(i - 1)} to 1")
          println(s"relatedCopy($i) before: ${relatedCopy(i)}")
          relatedCopy(i) = relatedCopy(i).slice(0, i - 1) + '1' + relatedCopy(i).slice(i, pplCount)
        ...
        if (i < pplCount - 1 && relatedCopy(i + 1)(i) == '0') {
          // update relatedCopy(i+1)(i) to '1'
          println(s"Need to update relatedCopy(${i + 1})($i) from ${relatedCopy(i + 1)(i)} to 1")
          println(s"relatedCopy(${i + 1}) before: ${relatedCopy(i + 1)}")
          relatedCopy(i + 1) = relatedCopy(i + 1).slice(0, i) + '1' + relatedCopy(i + 1).slice(i + 1, pplCount)
        ...
        if (i < pplCount - 1 && relatedCopy(i)(i + 1) == '0') {
          // update relatedCopy(i)(i+1) to '1'
          println(s"Need to update relatedCopy($i)(${i + 1}) from ${relatedCopy(i)(i + 1)} to 1")
          println(s"relatedCopy($i) before: ${relatedCopy(i)}")
          relatedCopy(i) = relatedCopy(i).slice(0, i + 1) + '1' + relatedCopy(i).slice(i + 2, pplCount)
          println(s"relatedCopy($i) after: ${relatedCopy(i)}")
        }
      }
    }
